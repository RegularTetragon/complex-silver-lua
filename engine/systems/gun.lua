...
gun.__index = gun
function gun:new(default_ammo, fire_delay, cons_proj, owner, fire_sound, owner_knockback)
    return setmetatable({
        ammo = default_ammo,
...
function gun:update(e, dt)
    ...
    if self.shot_timer < 0 and self.owner.controls.fire then
        self.owner.rb.v = self.owner.rb.v +
                              v2:new(e.trans.mirror_x and self.owner_knockback or -self.owner_knockback,
