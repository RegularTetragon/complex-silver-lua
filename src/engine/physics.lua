...
function col:deintersect(e, from_e)
 ...
 local s_t, s_b, s_r, s_l,
 				  f_t, f_b, f_r, f_l = 
  s_ub.y, s_lb.y, s_ub.x, s_lb.x,
  f_ub.y, f_lb.y, f_ub.x, f_lb.x
	assert(s_t > s_b)
	assert(s_r > s_l)
	assert(f_t > f_b)
	assert(f_r > f_l)
 ...
 if s_t > f_b and s_t < f_t then
  add(options, v2:new(0, f_b - s_t))
 end
 --need to move up (down in game)
 if s_b < f_t and s_b > f_b then
		add(options, v2:new(0, f_t - s_b)) 
 end
 if true then
...
local weld = system {name="weld", priority=2}
weld.__index = weld
...
function weld:update(e, dt)
 local mod_offset = v2:new(
  self.offset.x 
  *
  (
   self.to.trans.mirror_x
   and
   -1
   or
   1
